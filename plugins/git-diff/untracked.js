...
const MAX_UNTRACKED_TOTAL_BYTES = 2 * 1024 * 1024;   // bytes read per scan
...
async function readRegularFile(abs) {
  ...
  try {
    ...
    // The file can grow after stat. Read at most the limit plus one byte.
    const buf = Buffer.alloc(MAX_UNTRACKED_BYTES + 1);
    let length = 0;
    ...
    if (length > MAX_UNTRACKED_BYTES) return { oversizedBytes: length };
    return { buf: buf.subarray(0, length) };
  } catch {
