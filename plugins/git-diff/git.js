...
// for their output. Kept apart from server.js so the plugin's own code reads as what it does with
...
// names are what the override is built from: driversFromProbe says so, and server.js stops there.
//
// No folder's own config gets to change the shape of the patch either. BASE_ARGS pins the a/ and
// b/ prefixes, which the long-line cap matches on and the patch viewer parses.
//
...
// "a/<path> b/<path>", and the patch viewer parses the same header, so a folder that changes it can
...
// Reads the output of `git config --list --includes -z`, whose records are "key\nvalue" separated
...
// through git config --get-regexp so the patterns stay under our control. --includes also covers filters defined in included and global config files.
//
...
// A plugin can be launched from a Git hook. Its inherited repository/index
// overrides must not redirect reads away from the folder selected in the panel.
for (const key of ['GIT_DIR', 'GIT_WORK_TREE', 'GIT_INDEX_FILE', 'GIT_COMMON_DIR', 'GIT_OBJECT_DIRECTORY', 'GIT_ALTERNATE_OBJECT_DIRECTORIES']) delete GIT_ENV[key];
...
// What a `git config --list --includes -z` result means, kept apart from the call so the rule can be
...
// and diffing that folder is safe. So a result we cannot read reports ok: false and server.js
...
async function probeFilterDrivers(cwd) {
  const repo = await makeGit()(['rev-parse', '--git-dir'], cwd);
  if (!repo.ok) return { ok: false, usable: [], rejected: [], message: 'Not a Git repository.' };
  return driversFromProbe(await makeGit()(['config', '--list', '--includes', '-z'], cwd));
}
...
async function listWorktrees(git, repoRoot) {
  ...
  if (!listed.ok) throw new Error('Git worktrees could not be listed.');
  const trees = [];
  let current = null;
...
// The untracked paths, sorted so the synthesised patch is stable between polls.
// Report failures so the panel cannot mistake an incomplete result for a clean tree.
async function listUntracked(git, repoRoot) {
  const listed = await git(['ls-files', '--others', '--exclude-standard', '-z'], repoRoot);
  if (!listed.ok) throw new Error('Untracked files could not be listed or the file list is too large.');
